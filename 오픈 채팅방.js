...
function solution(record) {
  ...
  for (let message of record) {
    ...
    // recordObj[splitedMessage[1]] = splitedMessage[2];
    // 내가 작성한 코드는 Leave 시에도 닉네임을 업데이트 하여 undefined를 넣어 원하는 출력이 나오지 않을 수 있음
    // 예를 들어, 한 유저가 나가고 다시 들어오지 않은 경우 닉네임을 undefined로 지정해버림
    // 따라서 아래 코드처럼 수정해야 원하는 결과를 얻을 수 있음

    if (splitedMessage[0] != "Leave")
      recordObj[splitedMessage[1]] = splitedMessage[2];
  }
  ...
  for (let message of record) {
    ...
    if (message.startsWith("Enter")) {
      answer.push(`${recordObj[splitedMessage[1]]}님이 들어왔습니다.`);
    }
    if (message.startsWith("Leave")) {
      answer.push(`${recordObj[splitedMessage[1]]}님이 나갔습니다.`);
    }
  }
