...
function solution(k, score) {
  ...
  // 명예의 전당 상위 k번째를 기준으로 반복
  // k번째 이하라면 honor를 내림차순으로 정렬한 것의 마지막 요소를 answer 추가
  // k번째 이후라면 honor을 내림차순으로 정렬하고 배열의 길이를 유지하기 위해 honor의 마지막 요소를 pop
  // pop한 후 honor의 길이는 k로 유지되므로 거기서 제일 마지막 요소를 answer에 추가
  score.map((s) => {
    if (honor.length !== k) {
      honor.unshift(s);
      honor.sort((a, b) => b - a);
